...
$sql = "SELECT * FROM Doktor WHERE DoktorID = $id";
$result = mysqli_query($conn, $sql);
$doktor = mysqli_fetch_assoc($result);

if (!$doktor) {
    header('Location: index.php?error=Doktor bulunamadı');
    exit;
}
?>
...
<head>
    ...
    <title>Doktor Düzenle - Hastane Otomasyon Sistemi</title>
                ...
                <select id="brans_id" name="brans_id" required>
                    ...
                    <?php while ($brans = mysqli_fetch_assoc($branslar)): ?>
                        <option value="<?php echo $brans['BransID']; ?>" 
                                <?php echo $brans['BransID'] == $doktor['BransID'] ? 'selected' : ''; ?>>
